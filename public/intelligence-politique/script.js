...
function lineChart(canvas,sets,labels){if(!canvas||!canvas.offsetParent)return;const [c,w,h]=setupCanvas(canvas);c.clearRect(0,0,w,h);const pad={l:34,r:18,t:18,b:27},iw=w-pad.l-pad.r,ih=h-pad.t-pad.b;c.strokeStyle=cssVar('--chart-grid');c.lineWidth=1;for(let i=0;i<5;i++){let y=pad.t+ih*i/4;c.beginPath();c.moveTo(pad.l,y);c.lineTo(w-pad.r,y);c.stroke()}c.font='9px DM Sans';c.fillStyle=cssVar('--chart-axis');labels.forEach((l,i)=>c.fillText(l,pad.l+iw*i/(labels.length-1)-7,h-8));sets.forEach(s=>{c.strokeStyle=s.color;c.lineWidth=2.2;c.beginPath();s.data.forEach((v,i)=>{let x=pad.l+iw*i/(s.data.length-1),y=pad.t+ih*(100-v)/100;i?c.lineTo(x,y):c.moveTo(x,y)});c.stroke();s.data.forEach((v,i)=>{let x=pad.l+iw*i/(s.data.length-1),y=pad.t+ih*(100-v)/100;c.fillStyle=s.color;c.beginPath();c.arc(x,y,2.5,0,Math.PI*2);c.fill()})})}
...
/* Changement de thème : on redessine uniquement les canvas, sans toucher aux filtres, au zoom ni aux données. */
window.BFTheme.onChange(()=>{drawTrend();drawSentiment();drawRadar();drawNetwork()});
renderAll();
