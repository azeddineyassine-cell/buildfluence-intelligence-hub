(() => {
  ...
  <div class="ss-view" data-ss-view="orbits"><div class="ss-orbits"><div class="ss-orbits-head"><div><h2 data-ss="orbitTitle"></h2><p data-ss="orbitIntro"></p></div><label class="ss-actor-select"><span data-ss="actor"></span><select id="ss-actor-pick"></select></label></div><svg class="ss-topic-orbit" id="ss-topic-orbit" viewBox="0 0 820 470"></svg></div></div></div>
  ...
  function renderPlane() {
    ...
    rows.forEach((item,i)=>{const d=infoFor(item.name),b=balanceOf(d),btn=document.createElement('button'),jitter=((i%5)-2)*1.1;btn.className=`ss-bubble ${item.kind}`;btn.style.left=Math.max(4,Math.min(96,50+b/2+jitter))+'%';btn.style.bottom=Math.max(5,Math.min(94,7+d.visibility*.84+((i%3)-1)*1.5))+'%';btn.style.setProperty('--s',Math.min(72,34+Math.sqrt(d.urls)*1.25)+'px');btn.innerHTML=`<span>${d.short}</span><small>${fmt(d.urls)}</small>`;btn.title=`${label(d.name)} · ${fmt(d.urls)} URL`;btn.onclick=()=>select(item.name);plane.appendChild(btn);});
  }

  function eligibleActors(){return actors.filter(x=>state.active.has(x.actorId.startsWith('party_')?'party':'leader'));}
  function renderActorPicker(){const selectEl=root.querySelector('#ss-actor-pick'),eligible=eligibleActors();if(!eligible.some(x=>x.name===state.orbitActor))state.orbitActor=eligible[0]?.name||'';selectEl.innerHTML=eligible.map(x=>`<option value="${x.name}" ${x.name===state.orbitActor?'selected':''}>${x.name}</option>`).join('');selectEl.disabled=!eligible.length;}
  ...
  root.querySelector('#ss-actor-pick').addEventListener('change',e=>{state.orbitActor=e.target.value;renderOrbits();});
