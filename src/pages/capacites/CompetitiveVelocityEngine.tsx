    ...
    <DetailPageLayout>
      <div className="w-full bg-[#0F172A] text-white min-h-screen font-['Inter'] pb-32">
        ...
        {isFormOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-white text-slate-900 w-full max-w-lg rounded-[40px] p-10 shadow-2xl relative">
              ...
              <p className="text-slate-400 text-sm uppercase tracking-widest font-bold mb-8 italic">
                Competitive Velocity Engine • Buildfluence
              </p>
              <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                ></textarea>
                <button className="w-full bg-[#0D1B2A] text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-[#C9A84C] transition-all shadow-xl shadow-[#C9A84C]/10">
        ...
        <div className="text-center py-24 px-6">
          ...
          <h1 className="text-4xl md:text-6xl font-black mb-8 italic uppercase leading-tight tracking-tighter">
            ...
            {steps.map((s, i) => (
              <div key={i} className="relative z-10">
                <button
                  ...
                  className={`w-full flex items-center gap-6 p-6 rounded-[24px] transition-all duration-500 border-2 ${openStep === i ? "bg-white/10 border-[#C9A84C] shadow-[0_0_40px_rgba(201,168,76,0.15)]" : "bg-white/5 border-transparent hover:bg-white/10"}`}
                >
                  <div
                    style={{ background: s.color }}
                    className="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-black text-lg shrink-0 shadow-lg italic"
                  >
                    {s.num}
                  </div>
                ...
                {openStep === i && (
                  <div
                    className="md:ml-24 mt-4 p-8 bg-white/5 rounded-[24px] border-l-4 animate-in slide-in-from-top-4 duration-500"
                    style={{ borderColor: s.color }}
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                      {s.items.map((item, j) => (
                        <div key={j} className="text-sm text-slate-300 flex items-center gap-3 italic font-medium">
        ...
        <div className="flex justify-center mt-28 px-6">
          <button
            ...
            className="group relative bg-[#C9A84C] text-[#0F172A] px-16 py-6 rounded-full font-black uppercase text-xs tracking-[0.3em] overflow-hidden transition-all hover:scale-105 shadow-[0_20px_50px_rgba(201,168,76,0.3)]"
          >
            ...
            <span className="relative z-10 flex items-center gap-4 group-hover:text-[#C9A84C] transition-colors">
              Lancer le moteur <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
            </span>
          </button>
        </div>
      </div>
