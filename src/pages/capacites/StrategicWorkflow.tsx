      ...
      >
        {cycle.map((c, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center" }}>
            ...
            {i < cycle.length - 1 && (
              <div style={{ fontSize: 16, color: "#C9A84C", margin: "0 8px", paddingBottom: 20, opacity: 0.7 }}>→</div>
            )}
          </div>
        ))}
      </div>
          ...
          >
            ...
            {openStep === i && (
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: "0.5px solid rgba(13,27,42,.07)" }}>
                <ul style={{ listStyle: "none", padding: 0, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                  {s.items.map((item, j) => (
                    <li
                      key={j}
                      style={{
