  ...
  return (
    <div style={{ background: "#F4F1EA", minHeight: "100vh", fontFamily: "DM Sans,sans-serif" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", position: "sticky", top: 20 }}>
          <div style={{ width: "83%", maxWidth: 986, margin: 0, marginTop: -40, marginBottom: -40 }}>
            ...
            <svg viewBox="80 80 400 400" preserveAspectRatio="xMidYMid meet" style={{ width: "100%", height: "auto", display: "block", overflow: "visible" }}>
              ...
              <text x="280" y="274" textAnchor="middle" fontFamily="Playfair Display, serif" fontSize="22" fontWeight="900" fontStyle="italic" fill="#D9BC6A" letterSpacing="-0.3" style={{ cursor: "pointer" }} onClick={() => setSel(null)}>{lang === "en" ? "You are" : "Vous êtes"}</text>
              <text x="280" y="304" textAnchor="middle" fontFamily="Playfair Display, serif" fontSize="34" fontWeight="900" fontStyle="italic" fill="#D9BC6A" style={{ cursor: "pointer" }} onClick={() => setSel(null)}>{lang === "en" ? "here" : "là"}</text>
              {TOPICS.map(top => {
      ...
      <div style={{ background: "#0A1628", padding: "56px 24px", textAlign: "center", marginTop: 40 }}>
        <div style={{ fontFamily: "JetBrains Mono,monospace", fontSize: 10, fontWeight: 600, letterSpacing: "0.25em", textTransform: "uppercase", color: "#C9A84C", borderBottom: "1px solid #C9A84C", paddingBottom: 3, marginBottom: 20, display: "inline-block" }}>{t("Première étape", "First Step")}</div>
        <h2 style={{ fontFamily: "Playfair Display,serif", fontSize: "clamp(24px,3vw,38px)", fontWeight: 700, color: "#fff", lineHeight: 1.15, marginBottom: 10 }}>{lang === "en" ? <>Ready to <em style={{ fontStyle: "italic", color: "#C9A84C" }}>take action</em> ?</> : <>Prêt à <em style={{ fontStyle: "italic", color: "#C9A84C" }}>passer à l'action</em> ?</>}</h2>
        <p style={{ fontFamily: "Cormorant Garamond,serif", fontStyle: "italic", fontSize: 18, color: "rgba(255,255,255,.65)", maxWidth: 520, margin: "0 auto 28px", lineHeight: 1.7 }}>{t("Un échange de 30 minutes suffit pour cartographier votre exposition réelle.", "A 30-minute exchange is enough to map your real exposure.")}</p>
        <a href="#" onClick={(e) => { e.preventDefault(); setDiagOpen(true); }} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "#C9A84C", color: "#0A1628", fontFamily: "JetBrains Mono,monospace", fontSize: 11, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", padding: "14px 32px", borderRadius: 2, textDecoration: "none", cursor: "pointer" }}>{t("Passer à l'action", "Take action")} &#8594;</a>
      </div>
