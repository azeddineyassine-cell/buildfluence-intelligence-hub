...
const content: Record<Language, { steps: Step[]; cta: string }> = {
  fr: {
    ...
    cta: "ACTIVER MON PARCOURS D’IMPLANTATION",
  },
  en: {
    steps: [
      {
    ...
    cta: "ACTIVATE MY ESTABLISHMENT JOURNEY",
  },
};
...
const PositioningDecisionTrajectory = ({ hostDocument }: PositioningDecisionTrajectoryProps) => {
  ...
  const goToSimulator = () => {
    const simulatorLink = hostDocument.querySelector<HTMLElement>('.phase-sub a[data-target="cout"]');
    simulatorLink?.click();
  };
            ...
            <article className="pdt-step" style={{ "--pdt-rise": `${(2 - index) * 26}px` } as CSSProperties} key={step.number}>
              ...
              {expanded && (
                <div className="pdt-panel" id={panelId}>
                  ...
                  {index === 2 && <Button className="pdt-cta" type="button" onClick={goToSimulator}>{content[language].cta}</Button>}
                </div>
              )}
            </article>
          );
